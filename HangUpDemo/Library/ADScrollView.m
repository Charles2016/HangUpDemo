...
@interface ADScrollView ()<UIScrollViewDelegate> {
    ...
    UILabel * _pageLabel;
    UIView * _pageView;
    NSMutableArray *_tempArray;// 用来存放imageview，若是够就不创建，不够就补
}
@end
...
- (id)initWithFrameRect:(CGRect)rect imageArray:(NSArray *)imageArray titleArray:(NSArray *)titleArray {
    if ((self = [super initWithFrame:rect])) {
        ...
        _pageView = InsertView(self, CGRectMake(0, _scrollView.height - 30, _scrollView.width, 30), kColorBlack);
        _pageView.hidden = YES;
        _pageView.alpha = 0.8;
        _pageLabel = InsertLabel(_pageView, CGRectMake(0, 0, _pageView.width, _pageView.height), NSTextAlignmentCenter, @"", kFontSize11, kColorWhite, NO);
        // 重置广告栏信息
...
- (void)resetScrollViewInfo:(NSArray *)images {
    ...
    // 创建滚动广告栏
    _scrollView.contentSize = CGSizeMake(_adRect.size.width * _pageCount, _adRect.size.height);
    for (int i = 0; i < _pageCount; i++) {
        UIImageView *imageView;
        if (_tempArray.count > i) {
            imageView = _tempArray[i];
        } else {
        ...
        [imageView setFrame:CGRectMake(_adRect.size.width * i, 0.0, _adRect.size.width, _adRect.size.height)];
    ...
    _pageControl.numberOfPages = (_pageCount - 2);
    ...
    _pageLabel.text = [NSString stringWithFormat:@"%d/%lu",0,(_pageCount - 2)];
...
- (void)setPageControlFrame {
    if (PageControlCenter == _pageControlMode) {
        float pageControlWidth = (_pageCount - 2) * 10.0 + 40.;
...
- (void)setPageViewShow {
    _pageView.hidden = !_showPageView;
}
...
- (void)scrollViewDidScroll:(UIScrollView *)sender {
    if (_imageArray && 0 != _imageArray.count) {
        ...
        _pageLabel.text = [NSString stringWithFormat:@"%ld/%lu",(page-1),_imageArray.count-2];

...
+ (NSMutableAttributedString *)getColorsInLabelWithMiddleLine:(NSString *)allStr colorStrs:(NSArray *)colorStrs colors:(NSArray *)colors fontSizes:(NSArray *)sizes {
    ...
    if (colorStrs.count) {
        for (NSString *colorString in colorStrs) {
            ...
            [stringDict setObject:color forKey:NSForegroundColorAttributeName];
            [stringDict setObject:font forKey:NSFontAttributeName];
            [stringDict setObject:[NSNumber numberWithInteger:NSUnderlineStyleSingle] forKey:NSStrikethroughStyleAttributeName];
            [stringDict setObject:@(NSUnderlineStyleSingle) forKey:NSBaselineOffsetAttributeName];
            [allString setAttributes:stringDict range:stringRange];
        }
    }
    return allString;
...
+ (BOOL)setlimitPwdForTextField:(UITextField *)textField number:(int)number min:(BOOL)min {
    // 密码限制，输入8-16位
    if ([textField isFirstResponder] && !textField.text.length) {
...
+ (NSString *)objectToJsonString:(id)objct {
    ...
    NSError *error;
    NSData *jsonData = [NSJSONSerialization dataWithJSONObject:objct options:NSJSONWritingPrettyPrinted error:&error];
    NSString *jsonString;
    if (!jsonData) {
        DLog(@"%@",error);
    } else {

...
- (UIStatusBarStyle)preferredStatusBarStyle {
    return [self isTabbarRoot] ? UIStatusBarStyleLightContent : UIStatusBarStyleDefault;
}
...
- (void)setIsCurrentNavHide:(BOOL)isCurrentNavHide {
    ...
    self.sp_prefersNavigationBarHidden = isCurrentNavHide;
}
...
- (UIWindow *)keywindow {
    if (@available(*, iOS 13)) {
        ...
        return (UIWindow *)[UIApplication sharedApplication].windows.lastObject;
    }
}
...
#if DEBUG
    //InjectionIII调试代码
    [[NSNotificationCenter defaultCenter] addObserver:self selector:NSSelectorFromString(@"injected") name:@"INJECTION_BUNDLE_NOTIFICATION" object:nil];
#endif
...
- (void)loadViewData {
    ...
    self.navigationItem.leftBarButtonItem = [self isTabbarRoot] ? nil : [[self class] setItemsTitles:nil imageNames:@[@"nav_back_white"] isRightItems:NO titleColor:nil target:self action:@selector(backToSuperView)];
}
...
+ (UIViewController *)topViewController {
    ...
    if (@available(iOS 13, *)) {
        [[UIApplication sharedApplication].connectedScenes enumerateObjectsUsingBlock:^(UIScene *scene, BOOL *scenesStop) {
